...
def list_peers(lnd):
    print("Listing peers")
    peers = lnd.lightning_stub.ListPeers(ln.ListPeersRequest()).peers
    return peers
...
def channel_open_lifecycle(lnd1, lnd2, db, node_id):
    ...
    print("Channel opened: ", rev_txid)

    if rev_txid not in pending_open_channel_ids:
        raise AssertionError("Channel not found in pending channels")
    else:
        print("Channel observed in pending channels")
...
def close_all_channels(lnd):
    ...
    for channel in channels:
        ...
        channel_point = ln.ChannelPoint(funding_txid_str=channel.channel_point.split(':')[0], output_index=int(channel.channel_point.split(':')[1]))
        print("Channel point: ", channel_point)
        close_channel(lnd, channel_point)
    print("Closed all channels, mining block")
    mine_block()

def close_channel(lnd, channel_point, force=False):
    timeout_seconds = 5
    
    try:
        for response in lnd.lightning_stub.CloseChannel(ln.CloseChannelRequest(channel_point=channel_point, force=False), timeout=timeout_seconds):
            print("Close channel response: ", response)
            if response.close_pending:
                print("Channel close pending")
                return
    except grpc.RpcError as e:
        if e.details() == "channel close is already pending":
            print("Channel close already pending")
        elif e.details() == "unable to gracefully close channel while peer is offline (try force closing it instead): channel link not found":
            print("Peer offline, force close")
            close_channel(lnd, channel_point, force=True)
        else: 
...
def main():
    lnd1 = build_lnd1()
    print("Lnd1 built and has peer count: ", len(list_peers(lnd1)))
    lnd2 = build_lnd2()
    # lnd3 = build_lnd3()
    with psycopg.connect(conninfo="postgres://tsdbadmin:xlwo0rndweaduo66@x1wxu0povc.i8of53mz3a.tsdb.cloud.timescale.com:37927/tsdb") as conn:
